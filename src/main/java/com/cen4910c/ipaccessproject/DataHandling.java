...
public class DataHandling {
    ...
    @Transactional
    public User addUser(String firstName, String lastName, String zip, String email, String phoneNumber, String password){
        User user = new User(firstName, lastName, zip, email, phoneNumber, password);
    ...
    public void deleteAlertByID(int ID){
        ...
        if (alert != null) {entityManager.remove(alert);}
    }

    public void deleteReadAlerts(){
        String executeString = "DELETE FROM Alert d WHERE d.isRead = TRUE";
        Query query = entityManager.createQuery(executeString);
        int rowsAffected = query.executeUpdate();

        if (rowsAffected > 0) {System.out.println("Rows affected: " + rowsAffected);}
    }
}
